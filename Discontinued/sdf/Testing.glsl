...
float map(vec3 p) {
    ...
    float obj1 = sdBox2(p - vec3(-0.8, 0.3, 0.0), vec3(0.1,0.1,0.9));
    float obj2 = THEBOX(p-vec3(-0.5,0.0,0.0), vec3(0.7));
    return opSubtraction(obj1, obj2);
}
...
void main() {
    ...
    vec3 ro = vec3(-1.1, 0.7, 3); // Camera position

    float angleX = radians(13.0); // Rotation angle around X-axis
    float angleY = radians(18.0); // Rotation angle around Y-axis
